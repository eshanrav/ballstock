...
import { TrendingUp, TrendingDown, Activity, Search, Trophy, Briefcase, BarChart2, User, X } from "lucide-react"
...
function generatePriceHistory(basePrice: number, points = 30) {
  ...
  for (let i = 0; i < points; i++) {
    price = Math.max(1000, price + (Math.random() - 0.47) * (basePrice * 0.015))
    history.push({ time: `${points - i}m`, price: Math.round(price) })
  }
...
type Player = typeof INITIAL_PLAYERS[0]

function PlayerModal({ player, holding, onBuy, onSell, onClose }: {
  player: Player
  ...
  const history = useRef(generatePriceHistory(player.price)).current
  const isUp = player.change >= 0
  const color = isUp ? "#22c55e" : "#ef4444"
  const startPrice = history[0].price
  const pctFromStart = (((player.price - startPrice) / startPrice) * 100).toFixed(2)

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-lg p-6 relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            ...
            <div className="text-white/40 text-sm">{player.team} · {player.position}</div>
          </div>
          {holding && (
            <div className="ml-auto bg-green-500/10 text-green-400 text-sm px-3 py-1.5 rounded-full border border-green-500/20">
              {holding.shares} shares owned
            </div>
          )}
        </div>

        <div className="mb-4">
          <div className="text-3xl font-bold">{player.price.toLocaleString()} <span className="text-base font-normal text-white/40">chips</span></div>
          <div className={`text-sm mt-1 ${isUp ? "text-green-400" : "text-red-400"}`}>
            {isUp ? "▲" : "▼"} {isUp ? "+" : ""}{player.change} ({isUp ? "+" : ""}{player.changePct}%) today
            <span className="text-white/30 ml-3">{parseFloat(pctFromStart) >= 0 ? "+" : ""}{pctFromStart}% last 30 min</span>
          </div>
        </div>

        <div className="h-44 mb-5 -mx-2">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={history}>
              <XAxis dataKey="time" tick={{ fill: "rgba(255,255,255,0.3)", fontSize: 10 }} tickLine={false} axisLine={false} interval={9} />
              <YAxis domain={["auto", "auto"]} hide />
              <Tooltip
                contentStyle={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", fontSize: "12px", color: "#fff" }}
                formatter={(val: number) => [`${val.toLocaleString()} chips`, "Price"]}
              />
              <ReferenceLine y={startPrice} stroke="rgba(255,255,255,0.08)" strokeDasharray="4 4" />
              <Line type="monotone" dataKey="price" stroke={color} strokeWidth={2.5} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="bg-white/5 rounded-xl p-3 text-center">
            <div className="text-white/40 text-xs mb-1">Open</div>
            <div className="font-semibold text-sm">{(player.price - player.change).toLocaleString()}</div>
          </div>
          <div className="bg-white/5 rounded-xl p-3 text-center">
            <div className="text-white/40 text-xs mb-1">High</div>
            <div className="font-semibold text-sm text-green-400">{Math.max(...history.map(h => h.price)).toLocaleString()}</div>
          </div>
          <div className="bg-white/5 rounded-xl p-3 text-center">
            <div className="text-white/40 text-xs mb-1">Low</div>
            <div className="font-semibold text-sm text-red-400">{Math.min(...history.map(h => h.price)).toLocaleString()}</div>
          </div>
        </div>

        {holding && (
          <div className="bg-white/5 rounded-xl p-3 mb-4 flex justify-between text-sm">
            <span className="text-white/40">Your avg cost</span>
            <span className="font-medium">{holding.avgCost.toLocaleString()} chips</span>
            <span className={player.price >= holding.avgCost ? "text-green-400" : "text-red-400"}>
              {player.price >= holding.avgCost ? "+" : ""}{(player.price - holding.avgCost).toLocaleString()} per share
            </span>
          </div>
        )}

        <div className="flex gap-3">
          <button onClick={() => { onSell(); onClose() }} className="flex-1 py-3 rounded-xl border border-red-500/30 text-red-400 hover:bg-red-500/10 font-medium transition-colors">
            Sell
          </button>
          <button onClick={() => { onBuy(); onClose() }} className="flex-1 py-3 rounded-xl bg-green-500 hover:bg-green-400 text-black font-bold transition-colors">
            Buy
          </button>
        </div>
      </div>
...
export default function MarketPage() {
  ...
  const handleBuy = (player: Player) => {
    if (chips < player.price) { showToast("Not enough chips!", "sell"); return }
  ...
  const handleSell = (player: Player) => {
    ...
    if (!holding || holding.shares === 0) { showToast("You don't own this stock!", "sell"); return }
    setChips(c => c + player.price)
    setPortfolio(p => {
      const newShares = p[player.id].shares - 1
      if (newShares === 0) { const u = { ...p }; delete u[player.id]; return u }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      <nav className="border-b border-white/10 px-6 py-3 flex items-center justify-between sticky top-0 bg-[#0a0a0a]/95 backdrop-blur z-40">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <p className="text-white/50 mb-6">Trade stock in your favorite NBA players · <span className="text-green-400/60 text-xs">Click any card to see chart</span></p>
        ...
        <div className="flex items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center gap-2 text-sm text-white/50">
            Sort by:
            <select className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none" value={sortBy} onChange={e => setSortBy(e.target.value)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(player => {
            ...
            return (
              <div
                key={player.id}
                className="bg-white/5 border border-white/10 rounded-xl p-5 hover:border-white/25 transition-all cursor-pointer"
              ...
              >
                ...
                <div className="flex items-end justify-between">
                  ...
                  <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                    <button onClick={() => handleSell(player)} className="px-4 py-2 rounded-lg border border-red-500/30 text-red-400 hover:bg-red-500/10 text-sm font-medium transition-colors">
                      Sell
                    </button>
                    <button onClick={() => handleBuy(player)} className="px-4 py-2 rounded-lg bg-green-500 hover:bg-green-400 text-black text-sm font-bold transition-colors">
                      Buy
                    </button>
                  </div>
                </div>
      ...
      {selectedPlayer && (
        <PlayerModal
          ...
          holding={portfolio[selectedPlayer.id]}
          onBuy={() => handleBuy(selectedPlayer)}
          onSell={() => handleSell(selectedPlayer)}
          onClose={() => setSelectedPlayer(null)}
        />
      )}

      {toast && (
        <div className={`fixed bottom-6 right-6 px-5 py-3 rounded-xl text-sm font-medium shadow-lg z-50 ${toast.type === "buy" ? "bg-green-500 text-black" : "bg-red-500 text-white"}`}>
          {toast.msg}
        </div>
      )}
    </div>
